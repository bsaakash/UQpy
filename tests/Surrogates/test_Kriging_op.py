...
from UQpy.Surrogates import Kriging_op

dist = Uniform(loc=0, scale=5)
samples = MCS(dist_object=dist, nsamples=20, random_state=0).samples
values = np.cos(samples)
krig = Kriging_op(reg_model='Linear', corr_model='Gaussian', corr_model_params=[1], bounds=[[0.01, 5]], random_state=1)
krig.fit(samples=samples, values=values, nopt=100, corr_model_params=[1])

krig2 = Kriging_op(reg_model='Constant', corr_model='Gaussian', corr_model_params=[1], bounds=[[0.01, 5]],
...
linear_regression_model = Kriging_op(reg_model='Linear', corr_model='Gaussian', corr_model_params=[1]).reg_model
gaussian_corrleation_model = Kriging_op(reg_model='Linear', corr_model='Gaussian', corr_model_params=[1]).corr_model

krig3 = Kriging_op(reg_model=linear_regression_model, corr_model=gaussian_corrleation_model, corr_model_params=[1],
...
def test_reg_model():
    ...
    with pytest.raises(NotImplementedError):
        Kriging_op(reg_model='A', corr_model='Gaussian', corr_model_params=[1])
...
def test_corr_model():
    ...
    """
    with pytest.raises(NotImplementedError):
        Kriging_op(reg_model='Linear', corr_model='A', corr_model_params=[1])
...
def test_corr_model_params():
    ...
    """
    with pytest.raises(NotImplementedError):
        Kriging_op(reg_model='Linear', corr_model='Gaussian', bounds=[[0.01, 5]], nopt=100, random_state=1)
...
def test_optimizer():
    ...
    with pytest.raises(TypeError):
        Kriging_op(reg_model='Linear', corr_model='Gaussian', corr_model_params=[1], optimizer='A')
...
def test_random_state():
    ...
    """
    with pytest.raises(TypeError):
        Kriging_op(reg_model='Linear', corr_model='Gaussian', corr_model_params=[1], random_state='A')
...
#     """
#     with pytest.raises(NotImplementedError):
#         krig4 = Kriging_op(reg_model=linear_regression_model, corr_model=gaussian_corrleation_model, corr_model_params=[1],
#                            normalize=False)
#         krig4.fit(samples=samples[:4], values=values[:4])
